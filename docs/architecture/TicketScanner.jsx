...
const TicketScanner = () => {
  ...
  const validateTicket = async (qrCode) => {
    try {
      const response = await fetch('http://localhost:5000/api/tickets/validate-qr', {
        ...
        body: JSON.stringify({
          qrCode,
          validatorId: 'CURRENT_USER_ID', // À remplacer par l'ID du validateur connecté
        }),
      });
  ...
  const simulateScan = () => {
    ...
    const mockQrCode = 'eyJ0aWNrZXQiOiJUS1QtQUJDMTIzIiwiZXZlbnQiOiI2NTRhYmMxMjM0NSIsIm1lbWJlciI6IjY1NGFiYzEyMzQ1IiwidGltZXN0YW1wIjoxNzAwMDAwMDAwMDAwfQ==';
    ...
    <div className="min-h-screen bg-background-dark text-white font-display flex flex-col">
      ...
      <header className="fixed top-0 w-full z-20 bg-gradient-to-b from-black/80 to-transparent">
        <div className="flex items-center justify-between px-4 py-4">
          ...
          <h1 className="text-lg font-bold tracking-tight uppercase drop-shadow-md">
            Ticket Scanner
          </h1>
          
          <button
            onClick={toggleFlash}
            className={`p-2 rounded-full backdrop-blur-md text-white border transition-colors ${
              flashOn
      ...
      <main className="flex-1 flex flex-col h-full relative pt-16">
        ...
        <div className="relative h-[55%] bg-black w-full overflow-hidden">
          {isScanning ? (
            ...
            <div className="w-full h-full flex items-center justify-center bg-gray-900">
              <button
                onClick={startCamera}
                className="flex flex-col items-center gap-3 px-6 py-4 bg-primary text-black rounded-xl font-bold transition-transform active:scale-95"
              >
                <Camera size={32} />
          ...
          {isScanning && !validationResult && (
            <div className="absolute bottom-8 w-full text-center">
              ...
              <button
                ...
                className="px-6 py-2 bg-primary/20 border border-primary rounded-lg text-primary font-medium hover:bg-primary/30 transition-colors"
              >
                Simulate Scan (Demo)
              </button>
            </div>
        ...
        {/* Results Panel */}
        <div className="flex-1 bg-card-dark rounded-t-[2rem] -mt-6 relative z-10 p-6 flex flex-col shadow-[0_-10px_40px_rgba(0,0,0,0.5)] border-t border-white/5">
          ...
          {validationResult ? (
            ...
            <div className="flex flex-col items-center justify-center flex-1 space-y-4">
              ...
              <button
                ...
                className="mt-4 px-8 py-3 bg-primary text-black rounded-xl font-bold hover:bg-primary/90 transition-all active:scale-95"
              >
                Scan Next Ticket
              </button>
            </div>
      ...
      {showManualEntry && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-6">
          <div className="w-full max-w-md bg-card-dark rounded-2xl p-6 border border-white/10">
            ...
            <input
              ...
              className="w-full px-4 py-3 bg-black/20 border border-white/10 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-primary"
              autoFocus
            />
            <div className="flex gap-3 mt-6">
              ...
              <button
                ...
                className="flex-1 py-3 bg-primary text-black rounded-xl font-bold hover:bg-primary/90 transition-colors"
              >
                Validate
              </button>
            </div>
